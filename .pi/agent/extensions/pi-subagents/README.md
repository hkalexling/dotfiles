...
> **⚠️ Locally-modified fork.** This is a local clone extracted from the npm package and customized.
> It lives at `~/.pi/agent/extensions/pi-subagents/` and is loaded as a pi extension via auto-discovery.
>
> **Changes from upstream v17.2.0:**
>
> | Area | Change |
> |------|--------|
> | Centralized config | Added `defaultModel`, `defaultThinking`, `agents` fields to `~/.pi/agent/subagents.json` so you can set model/thinking defaults for all subagents without editing individual agent `.md` files. |
> | Precedence | Tool call params now override agent frontmatter for `model`/`thinking` (was: frontmatter locked them). New order: params → per-agent settings → global settings → frontmatter → parent session. |
> | Deep merge | `agents` map in `subagents.json` is deep-merged across global/project layers, so project can override just `thinking` without losing the global `model` for an agent. |
> | Save safety | `/agents → Settings` no longer wipes hand-edited `defaultModel`/`defaultThinking`/`agents` from the project file — reads existing file first, merges menu fields in. |
> | Error surfacing | Invalid model strings from `subagents.json` produce visible errors instead of silent fallback to parent. |
> | Tests | Updated `invocation-config.test.ts` (rewritten for new precedence), `settings.test.ts` (+11 new tests), `spawn-config.test.ts`, and test helpers. Added `tsconfig.json`, dev dependencies, and test harness. |
> | Docs | Added `docs/decisions/0005-centralized-model-thinking-defaults.md`. Updated README sections: Frontmatter Fields, Persistent Settings. |
...
| `model`             | centralized config → parent | Model — `provider/modelId` or fuzzy name (`"haiku"`, `"sonnet"`). Falls back through subagents.json settings, then parent model.                                                                                                                                        |
| `thinking`          | centralized config → parent | off, minimal, low, medium, high, xhigh. Falls back through subagents.json settings, then parent.                                                                                                                                                                                         |
...
1. `subagent()` tool call params (LLM-specified at call time)
2. Per-agent override in `subagents.json` (`agents.<name>`)
3. Global default in `subagents.json` (`defaultModel` / `defaultThinking`)
4. Agent `.md` frontmatter / built-in defaults
5. Parent session model / thinking

Frontmatter is authoritative for other fields (`max_turns`, `inherit_context`, `run_in_background`):
if an agent file sets them, `subagent` tool parameters cannot override.
...
All settings are stored in JSON files, merged on load:
...
- **Project:** `<cwd>/.pi/subagents.json` — per-project overrides.
  Written by `/agents` → Settings (menu fields only).

**Precedence:** project overrides global on any field present in both.
For `agents` (per-agent overrides), project entries are deep-merged into global — you can override just `thinking` for `Explore` without losing the global `model`.

### Operational fields (menu-managed)

| Field | Default | Description |
|-------|---------|-------------|
| `maxConcurrent` | `4` | Max concurrent background agents |
| `defaultMaxTurns` | `0` (unlimited) | Max turns before wrap-up |
| `graceTurns` | `5` | Grace turns after wrap-up steer |

### Model/thinking defaults (hand-edit only)

| Field | Description |
|-------|-------------|
| `defaultModel` | Default model for all subagents — `provider/modelId` or fuzzy name (`"haiku"`, `"sonnet"`) |
| `defaultThinking` | Default thinking level for all subagents — `off`, `minimal`, `low`, `medium`, `high`, `xhigh` |
| `agents` | Per-agent overrides, keyed by agent name (e.g. `"Explore"`, `"Plan"`). Each entry can set `model` and/or `thinking`. |

These fields are NOT written by `/agents` → Settings — edit them by hand.
When saving menu fields, any hand-edited model/thinking fields in the project file are preserved.

**Precedence for `model` and `thinking`:**

1. `subagent()` tool call params
2. Per-agent `agents.<name>` in `subagents.json`
3. Global `defaultModel` / `defaultThinking` in `subagents.json`
4. Agent `.md` frontmatter / built-in defaults
5. Parent session

**Example — global defaults with per-agent overrides:**
