...
describe("settings persistence", () => {
  ...
  it("saveSettings preserves hand-edited model/thinking fields in project file", () => {
    writeProject({
      maxConcurrent: 4,
      defaultModel: "sonnet",
      agents: { Explore: { model: "haiku" } },
    });
    // Simulate /agents → Settings changing concurrency
    saveSettings({ maxConcurrent: 8, defaultMaxTurns: 30, graceTurns: 10 }, projectDir);
    const saved = JSON.parse(readFileSync(projectFile(), "utf-8"));
    expect(saved.maxConcurrent).toBe(8);
    expect(saved.defaultMaxTurns).toBe(30);
    expect(saved.graceTurns).toBe(10);
    // Hand-edited fields survive
    expect(saved.defaultModel).toBe("sonnet");
    expect(saved.agents).toEqual({ Explore: { model: "haiku" } });
  });
  ...
  describe("sanitizer", () => {
    ...
    it("accepts valid defaultModel string", () => {
      writeProject({ defaultModel: "sonnet" });
      expect(loadSettings(globalDir, projectDir)).toEqual({ defaultModel: "sonnet" });
    });

    it("accepts valid thinking levels", () => {
      for (const level of ["off", "minimal", "low", "medium", "high", "xhigh"]) {
        writeProject({ defaultThinking: level });
        expect(loadSettings(globalDir, projectDir)).toEqual({ defaultThinking: level });
      }
    });

    it("drops invalid thinking levels", () => {
      writeProject({ defaultThinking: "extreme" });
      expect(loadSettings(globalDir, projectDir)).toEqual({});
      writeProject({ defaultThinking: 5 });
      expect(loadSettings(globalDir, projectDir)).toEqual({});
    });

    it("accepts valid agents map", () => {
      writeProject({
        agents: { Explore: { model: "haiku", thinking: "low" } },
      });
      expect(loadSettings(globalDir, projectDir)).toEqual({
        agents: { Explore: { model: "haiku", thinking: "low" } },
      });
    ...
    it("drops agents entries with no model or thinking", () => {
      writeProject({
        agents: { Explore: { model: "haiku" }, Empty: {} },
      });
      expect(loadSettings(globalDir, projectDir)).toEqual({
        agents: { Explore: { model: "haiku" } },
      });
    ...
    it("returns undefined agents when all entries are empty", () => {
      writeProject({ agents: { A: {}, B: {} } });
      expect(loadSettings(globalDir, projectDir).agents).toBeUndefined();
    });

    it("deep-merges agents across layers", () => {
      writeGlobal({
        agents: { Explore: { model: "haiku", thinking: "low" } },
      });
      writeProject({
        agents: { Explore: { thinking: "minimal" } },
      });
      expect(loadSettings(globalDir, projectDir)).toEqual({
        agents: { Explore: { model: "haiku", thinking: "minimal" } },
      });
    });

    it("deep-merges agents — project adds new agent entries", () => {
      writeGlobal({
        agents: { Explore: { model: "haiku" } },
      });
      writeProject({
      ...
      expect(loadSettings(globalDir, projectDir)).toEqual({
        agents: { Explore: { model: "haiku" }, Plan: { model: "sonnet" } },
      });
    });
...
describe("SettingsManager", () => {
  ...
  describe("model/thinking defaults — getAgentDefaults", () => {
    let globalDir: string;
    let projectDir: string;

    beforeEach(() => {
      globalDir = mkdtempSync(join(tmpdir(), "pi-sm-model-global-"));
      projectDir = mkdtempSync(join(tmpdir(), "pi-sm-model-proj-"));
    });
    ...
    it("returns empty when no config exists", () => {
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      expect(sm.getAgentDefaults("Explore")).toEqual({});
    });

    it("returns global defaultModel for any agent", () => {
      writeFileSync(join(globalDir, "subagents.json"), JSON.stringify({ defaultModel: "sonnet" }));
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      expect(sm.getAgentDefaults("Explore")).toEqual({ model: "sonnet" });
      expect(sm.getAgentDefaults("general-purpose")).toEqual({ model: "sonnet" });
    });

    it("returns global defaultThinking for any agent", () => {
      writeFileSync(join(globalDir, "subagents.json"), JSON.stringify({ defaultThinking: "high" }));
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      expect(sm.getAgentDefaults("Explore")).toEqual({ thinking: "high" });
    });

    it("returns per-agent override over global default", () => {
      ...
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      expect(sm.getAgentDefaults("Explore")).toEqual({ model: "haiku" });
      expect(sm.getAgentDefaults("general-purpose")).toEqual({ model: "sonnet" });
    });

    it("merges per-agent thinking with global model", () => {
      writeFileSync(join(globalDir, "subagents.json"), JSON.stringify({
        defaultModel: "sonnet",
        defaultThinking: "high",
        agents: { Explore: { thinking: "low" } },
      }));
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      expect(sm.getAgentDefaults("Explore")).toEqual({ model: "sonnet", thinking: "low" });
    });

    it("project per-agent override merges with global per-agent (deep merge)", () => {
      writeFileSync(join(globalDir, "subagents.json"), JSON.stringify({
        agents: { Explore: { model: "haiku", thinking: "low" } },
      }));
      mkdirSync(join(projectDir, ".pi"), { recursive: true });
      ...
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      // Project overrides thinking, global model survives
      expect(sm.getAgentDefaults("Explore")).toEqual({ model: "haiku", thinking: "minimal" });
    });

    it("load() exposes defaultModel and defaultThinking", () => {
      mkdirSync(join(projectDir, ".pi"), { recursive: true });
      writeFileSync(join(projectDir, ".pi", "subagents.json"), JSON.stringify({
        defaultModel: "opus",
      ...
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
      expect(sm.defaultModel).toBe("opus");
      expect(sm.defaultThinking).toBe("xhigh");
    });
  });
