...
import { type InvocationSettings, resolveAgentInvocationConfig } from "#src/config/invocation-config";
 ...
 */
/** Narrow settings interface for spawn config resolution. */
export interface SpawnSettings extends InvocationSettings {
...
export function resolveSpawnConfig(
  ...
  const resolvedConfig = resolveAgentInvocationConfig(customConfig, params, settings);
  ...
  const resolution = resolveInvocationModel(
    ...
    resolvedConfig.modelFromSettings,
  );
