 ...
 *    - `modelFromSettings` true → return `{ error }` (user explicitly configured it in subagents.json).
 *    - Both false               → silent fallback to `parentModel` (came from agent frontmatter/builtin).
 */
export function resolveInvocationModel(
  ...
  if (modelFromParams || modelFromSettings) return { error: resolved };
